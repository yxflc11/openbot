...
所有权及解压/安装串行化、systemd 命令适配器、明确恢复命令，以及原生 x64/arm64 证据。无 root
安全解压适配器现已能拒绝危险清单、解压到私有空目录、复核压缩包摘要并重建候选 manifest；现有
x64 压缩包已在 Ubuntu 容器模拟中通过该路径。目前还没有接受过真实远程来源证明。
