...
高权限目录策略现在已经可以执行，但还没有接到命令入口。它固定所有目标路径，并要求 Linux 有效
uid/gid 均为 0、所有祖先由 root 持有且不是符号链接并且普通用户不可写、公开/私有子目录使用精确
权限模式，同时保证原子移动所需的文件系统关系。调用方不能改用其他目标路径或“可信所有者”。
...
引导程序分发、root 所有目录创建、经过审查的运维命令入口，以及 systemd 适配器的原生 x64/arm64
证据。无 root 安全
