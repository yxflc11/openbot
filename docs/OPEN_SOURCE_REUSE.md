...
| --- | --- | --- | --- |
| Server model chat and provider connections | [OpenAI Node SDK `7.10.0` / `c22b09bc`](https://github.com/openai/openai-node/tree/c22b09bc8bd5feed4ebd02b9f363a1a8b479b5ab), [Anthropic SDK `0.124.0` / `ba14b1f4`](https://github.com/anthropics/anthropic-sdk-typescript/tree/ba14b1f4fdf2e840a7b32297965342a099f6201d), and 11 reviewed provider API contracts | Apache-2.0; MIT; API reference only | Reuse released SDKs for bounded text chat and one-page discovery, with Owner-managed presets/custom endpoint authorization and immutable Run model selection. Kimi now permits the reviewed public Formula search/read tools; preserve no paid retries and no private-reasoning persistence. No source copied; see [connection research](research/model-service-presets.md) and [original Kimi review](research/kimi-model-chat.md). |
| Model public web research / 模型公开联网检索 | OpenAI SDK `7.10.0` / `c22b09bc`; Kimi Formula v1 observed 2026-09-08 | Apache-2.0; hosted API | Reuse official search/read formulas with fixed local authority, four-call budget and content-free audit. No source copied. See [research](research/model-web-search.md). |
| Shared model web tools | Existing OpenAI `7.10.0` and Anthropic `0.124.0`; Tavily API and Kimi Formula contracts observed 2026-09-08 | Apache-2.0; MIT; hosted APIs | Reuse SDK continuations and common bounded authority. Separate inference/retrieval credentials, return plaintext across providers, retain content-free audit. No source copied; see [research](research/shared-model-web-tools.md). |
| Model credential encryption | [NIST SP 800-38D](https://csrc.nist.gov/pubs/sp/800/38/d/final), [Node `v26.8.1` crypto](https://github.com/nodejs/node/blob/v26.8.1/doc/api/crypto.md), and the existing OpenSSH POSIX permission review | Open standard; MIT; BSD-style reference | Use Node AES-256-GCM, a random 96-bit nonce, 128-bit tag, and connection-bound AAD for PostgreSQL API keys. A separate POSIX `0600` key file must accompany database backups; missing keys with existing connections fail closed. No local cipher, source copying, native keyring, or KMS; see [research](research/model-service-presets.md). |
...
| Employee browser sessions | [CopilotKit/OpenBot `257c1280`](https://github.com/CopilotKit/openbot/tree/257c1280d684089be9adb0b35cce262efc7064bf), Playwright 1.62.1, Bun 1.4.2 and RFC 6455 | MIT; Apache-2.0; IETF Trust | Persistent profiles and human input behind Server-owned sessions, exclusive control, content-free audit and the outbound Node relay. Runtime source is unchanged; the adapted build recipe retains MIT attribution and license. See [research](research/employee-browser.md). |
...
| Server model connections, credentials, and Employee model selection | Reviewed | Eleven official API contracts, pinned OpenAI/Anthropic SDKs, NIST/Node encryption, and existing POSIX file boundaries are recorded. Discovery is one page/256 IDs/2 MiB; chat responses stay within 256 KiB. Runs retain their selected connection/model without silent fallback. Live-provider claims remain tied to authorized tests. |
...
- Owners can save, rotate, and disable encrypted model connections and select models for new or
  existing model Employees; endpoints and protocols remain immutable. Saving does not run paid
  inference, discovery is bounded, and testing is explicit. Custom endpoints require the Server
  exact HTTPS allowlist. Read-only `legacy-kimi` retains existing unbound behavior; credentials
  and local bindings never enter Employee exports.
