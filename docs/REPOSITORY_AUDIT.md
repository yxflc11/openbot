...
| --- | --- | --- |
| P1 | PostgreSQL CI covered automation/direct tasks but collaboration tests could remain skipped; reactions/removal added another skipped-by-default suite | Added explicit collaboration and interactions CI steps with separate disposable databases. All four local PostgreSQL suites pass, 49 tests. The isolated suites also passed in hosted CI. |
...
This review ran document-link validation and the 26-file migration manifest check, focused action/reaction component tests, Server/Web type checks, the Server regression suite and all four isolated PostgreSQL suites. The release integrator subsequently passed full root `npm run check`, rendered-demo checks and independent website deployment. Hosted CI passed security, PostgreSQL, both container architectures and macOS. [Windows native lifecycle validation](https://github.com/yxflc11/openbot/actions/runs/34497646235) also passed. Installer publication remains a separate, exact-source release gate; this audit does not certify physical devices.
