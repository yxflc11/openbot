...
- 每个 Bot 有固定角色形象、名字、职责和状态文字。角色使用 16×16 网格的三色 SVG 像素块语言，在 28px 头像和办公室大图中保持同一轮廓。
- 角色只保留方形屏幕脸与单一强调色，不使用拟真高光、复杂关节、手指和装甲分件；角色服务于状态识别，不能抢占任务信息。
- 状态只改变眼睛、胸前信号块与单一强调色；工作和等待时使用两帧阶梯动画，并遵循系统的“减少动态效果”设置。
...
| `FRAME_UPDATED` | Inspector 中的最新 Computer 预览；正文按 revision 鉴权读取 |
