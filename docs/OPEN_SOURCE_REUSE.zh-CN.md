...
| Linux Worker Host 高权限引导边界 | [Node.js `v22.22.2` / `2645dc73`](https://github.com/nodejs/node/tree/2645dc73720b1b4f27c49f395d3c66025ce126cc)、POSIX.1-2024 目录操作、Linux man-pages `6.19` `openat2`、未采用的 [`proper-lockfile` `4.1.2` / `9f8c303c`](https://github.com/moxystudio/node-proper-lockfile/tree/9f8c303c91998e8404a911dc11c54029812bca69)与未采用的 [`fs-ext` `2.1.1` / `aded9760`](https://github.com/baudehlo/node-fs-ext/tree/aded976099c2b06c944f0897a9b004dbf266e234) | Node.js 许可证；标准；man-pages 许可证集合；MIT；MIT | 通过已打开句柄把不可信压缩包只复制一次，放进路径固定、root 持有的私有状态目录；随后只在同一把原子目录锁下对这份导入文件执行来源证明、解压和激活。高权限陈旧锁不会按时间自动回收。Node core 未暴露 `openat2`；固定目标路径并要求所有可变祖先均由 root 持有且不可被组/其他用户写入，可在不增加原生二进制的情况下封闭第一段引导缺口。未复制上游源码，见[调研证据](research/linux-worker-host-privileged-bootstrap.md)。 |
