...
| Windows Worker Host 服务边界 | [`Microsoft.Extensions.Hosting.WindowsServices` `10.0.11`](https://www.nuget.org/packages/Microsoft.Extensions.Hosting.WindowsServices/10.0.11)，来自 [`dotnet/dotnet` `e2f47b01`](https://github.com/dotnet/dotnet/tree/e2f47b0110ed922f21a1522da67279133ce28f32)；Microsoft `CreateService` 与 LocalService 契约；拒绝 [WinSW `v2.12.0` / `eef5bade`](https://github.com/winsw/winsw/tree/eef5bade59fca0254e387ac73ed7625ba6aa7147) 和 [`node-windows` `1.0.0-beta.8` / `e60ec01c`](https://github.com/coreybutler/node-windows/tree/e60ec01cb63f73a713581548d24256b25c5f93f8) | MIT；Microsoft 文档/API；MIT；MIT 与随包组件 | 只采用 Microsoft 正式发布的 SCM 生命周期，并制作为自包含、单文件的 Windows x64 主机。本地只补齐固定且已验证的子进程、有界排空、进程树、脱敏诊断与安装事务差集。首版采用 LocalService，但在任何支持声明前必须取得原生身份、ACL、停机、重启、升级和凭证拒绝证据。拒绝 WinSW 是因为稳定版仍有受限账号/SCM 故障；`node-windows` 捆绑 WinSW/Sudowin，并可能记录生成的配置。未复制上游源码，见[调研证据](research/windows-worker-host-service.md)。 |
