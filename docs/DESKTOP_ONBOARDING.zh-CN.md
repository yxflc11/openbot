...
Desktop 0.1.0-alpha.3 在 macOS、Windows 和 Linux 共用同一个应用。macOS 源码构建预览提供两种用途；
...
输入框内的 **+** 添加可移除的附件与技能标签。最多添加 **3 个 UTF-8 TXT、MD、CSV 或 JSON 文件**，
每个最多 **6,000 字节**。文件内容作为用户提供的任务文字提交；正文、附件名称/内容和技能请求合计不得超过
**8,000 字符**。这不是二进制、PDF 或截图上传。最多请求 **2 个已分配给目标 Bot 且已验证的技能**；
技能选择只是使用请求，不授予能力，Server 在执行时重新检查技能分配和审核状态。

“分享”先明确预览最近读取消息的 Markdown 内容，点击“复制对话”后写入剪贴板，失败时可手动复制。
这不等于完整历史导出或托管公开链接，不会自动发布对话。
