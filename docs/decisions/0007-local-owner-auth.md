...
1. M0 采用单 Owner 模型。Owner 显示名和密码由 Server 环境变量提供；密码至少 12 个字符，不写入数据库。
2. 登录成功后生成 256-bit 随机 Token，通过 `HttpOnly`、`SameSite=Strict`、`Path=/` Cookie 返回。HTTPS 部署必须启用 `Secure`。
...
- 当前限速状态保存在单 Server 内存中，重启后清空；横向扩容前应迁移到共享限速存储。
