...
同步容器固定镜像、严格允许列表、真实 smoke 版本断言和双语当前文档。首次汇总检查还发现 Windows 工作流校验器保留旧 setup-dotnet SHA；同步严格固定值及负向测试，继续拒绝浮动标签和权限边界放宽。保留 npm 10.9.9、Node 24 LTS 路线、所有安全及原生测试，不扩大权限，不复制源码，不取消失败检查。setup-dotnet 的多 SDK 预览质量问题不涉及单一 global.json。builder 的密钥链、许可证、签名日志修复不代表 OpenBot 已具有正式签名证书。
