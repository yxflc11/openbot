...
## Standalone website repository

The product/website boundary was revised on 2026-09-10: `yxflc11/openbot-website` owns public pages, bilingual manuals and Pages deployment. OpenBot retains runtime components, canonical engineering contracts and plugin catalog metadata. The website builds the real demo from an exact 40-character OpenBot commit, with a disposable ignored checkout and locked dependencies. No generated JS bundle is committed and no arbitrary repository URL is accepted. This reuses Git and the existing released build tools, with no new runtime framework. Original monorepo-resolution research above is historical; the independent site does not need that cookie workaround. Verify a clean independent build and both repository links before removing the monorepo copy.

## Narrow embedded demo layout correction

Chrome and the in-app browser at 537 × 837 exposed a 68-pixel blank strip hiding the composer in the 560-pixel demo iframe. DOM measurements showed `body` inherited the product's mobile-navigation bottom padding while the demo has no mobile navigation: the 100% root was only 492 pixels high and clipped its 560-pixel child. Rechecked the existing stylesheet at c179447fd182ec73a082eb33e36fcda1729ba028 and [W3C CSS Sizing percentage/box sizing rules](https://www.w3.org/TR/2026/WD-css-sizing-3-20260904/). Reset padding only in the separate demo entry stylesheet, preserving the product navigation layout. No dependency or copied source. Validate the actual embedded composer at narrow and desktop sizes, not just its standalone document.
