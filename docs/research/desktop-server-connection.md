...
- GitHub queries: `electron v44.2.0 protocol.handle session.fetch`, Electron issue `45869`,
  `electron-store v11.0.2`, and each candidate's source, releases, tests, issues, and license.
...
- Selected upstream or standard: Electron 44.2.0 session-specific `protocol.handle`,
  ...
  `write-file-atomic` 8.0.0 dependency.
...
- Files and upstream locations: implementation will call public Electron and `write-file-atomic`
  APIs. The URL/config/request policy is OpenBot-specific; no candidate source or tests are copied.
- Required copyright or license notice location: Electron and `write-file-atomic` are already listed
  in `THIRD_PARTY_NOTICES.md`; the packaged dependency inventory must retain the ISC notice for
  `write-file-atomic` and the MIT notice for `signal-exit`.
