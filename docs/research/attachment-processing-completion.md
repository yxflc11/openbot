...
Windows CI exposed PDF.js `getFactoryUrlProp` rejecting the `standardFontDataUrl` filesystem directory because `fileURLToPath` returns a trailing backslash on Windows. Inspected the pinned 6.2.108 implementation: validation requires `/`, while the Node factory uses `fs.readFile`. Normalize the trusted dependency directory's separators to forward slashes, accepted by Windows Node filesystem APIs, preserving local-only reads. Existing real encrypted/OpenAction PDF tests are the cross-platform regression gate.
