...
| npm native SBOM | npm CLI `10.9.9` / `745d8d90b5403110d26ba332ba83d8c5a51f0578`; npm integrity `sha512-1g+6jLQvaIuB4zwvHL7yrXuXcWZwDsCtBX8bbWDqbvJSSr9nPiDDWTHNgwXR27iIcTTW7v3A57hDW9RYv2W4Yg==` | Artistic-2.0 | Released CLI with native SPDX 2.3 and CycloneDX support; this patch release updates `tar` to `7.5.22` and replaces the vulnerable `10.9.8` tool baseline | Avoids another generator dependency. The monorepo workspace view currently includes Node test-only workspace links, so release code must create and validate a production-only lock projection before calling it | Select the existing pinned package-manager capability behind a tested projection adapter |
...
- Selected upstream or standard: Node.js `v22.22.2` archives, `@vercel/ncc` `0.45.0`, npm SBOM
  `10.9.9`, SPDX 2.3 output, GNU tar 1.35, Ubuntu's security-patched xz 5.4.5 line, and
