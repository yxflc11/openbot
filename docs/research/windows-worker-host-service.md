...
| Redirected standard input | .NET 10 [`ProcessStartInfo.RedirectStandardInput`](https://learn.microsoft.com/en-us/dotnet/api/system.diagnostics.processstartinfo.redirectstandardinput?view=net-10.0) and [Node.js `v22.22.2` / `2645dc73`](https://github.com/nodejs/node/tree/2645dc73720b1b4f27c49f395d3c66025ce126cc) `process.stdin` | Microsoft documentation/API; Node.js license | Stable parent-child stream APIs, already present in the selected runtimes, with no new shipped dependency. | The service creates the anonymous pipe while starting the exact child. There is no named endpoint, command listener, credential, or general IPC surface. A first exact `START` frame keeps the trusted Node entry point inert until Job assignment succeeds; EOF gives a fail-closed parent-loss signal. | Select for an ordered versioned ASCII `START` then `SHUTDOWN` protocol with a 96-byte lifetime bound. Standard input is reserved for lifecycle control in Windows service mode only. |
...
| Kernel-time Job assignment | Windows [`CREATE_SUSPENDED`](https://learn.microsoft.com/en-us/windows/win32/procthread/process-creation-flags) or [`PROC_THREAD_ATTRIBUTE_JOB_LIST`](https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-updateprocthreadattribute) | Microsoft documentation/API | Stable native contracts; the Job List attribute requires Windows 10 or Windows Server 2016 and newer. .NET `ProcessStartInfo` does not expose either contract directly. | Eliminates the post-start assignment window at the kernel boundary, but OpenBot would own process creation, mutable command buffers, Unicode environment blocks, inheritable standard handles, attribute-list memory, thread resume, and every failure cleanup path. | Defer for the first prototype. Use the verified Node entry point's private startup gate, then require native tests proving no OpenBot client or Provider side effect occurs before assignment. Reconsider a maintained adapter or focused interop if that proof fails. |
...
- Exact OpenBot-specific gap: Bind SCM start/stop to one fixed verified release, keep the Node entry
  point inert until an ordered bounded startup frame follows successful Job assignment, parse the
  later shutdown frame from the same inherited standard input, make Node shutdown await cooperative
  Provider cancellation, terminate the unnamed non-breakaway job after the deadline, emit
  allowlisted diagnostics, and integrate the existing recoverable install states without granting
  authority.
...
- Automated tests: Pure lifecycle tests for fixed executable/argument selection, no client start
  before Job assignment, fragmented/exact/malformed/oversized/out-of-order standard-input frames,
  EOF, repeated stop, cooperative execution drain, bounded graceful stop, deadline job termination,
  child crash, restart, cancellation, redacted diagnostics, and install-transaction handoff.
  Windows integration tests must install, query, start, stop, restart, upgrade, roll back, reboot,
  and uninstall the real service.
...
- None for lifecycle transport: redirected standard input is the private inherited one-way channel.
  `stdio-v2` accepts only ordered exact `START` and `SHUTDOWN` frames within 96 lifetime bytes.
  Invalid, extra, excessive, out-of-order input or EOF before shutdown requests a non-zero
  fail-closed shutdown. The service sends `START` only after successful Job assignment.
