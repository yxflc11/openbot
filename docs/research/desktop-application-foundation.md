...
- Status: Accepted design; dependency incorporation and real-device evidence pending
- Date: 2026-09-04
...
- GitHub queries: `electron electron release 44.2.0`, `electron forge release 7.11.2`,
  `tauri release 2.11.5`, `wails release 2.14.0`, and each repository's source, release history,
  tests, issues, security surface, platform support, and license.
- Standards and primary documentation queries: Node.js release status and v22-to-v24 migration;
  ...
  code signing, updating, and Forge packaging; Tauri process model, capabilities, permissions,
  signing, and distribution; Wails platform and WebView requirements; Playwright Electron support.
...
| Electron Forge | [`v7.11.2`](https://github.com/electron/forge/releases/tag/v7.11.2), commit `f2a3ec8` | MIT | Current stable official Electron toolkit reviewed on 2026-09-04; maintained source, tests, issues, and signed releases. Forge 8 remains alpha. | Covers packaging, platform makers, signing, and publishing without introducing another application language. It does not provide OpenBot's security policy or release evidence by itself. | Select as the initial packaging adapter; pin exact packages when implementation begins. |
...
- Selected upstream or standard: Electron 44.2.0, Electron Forge 7.11.2, Electron's process and
  security contracts, Node.js 24.20.0 LTS, and the repository's existing TypeScript/React/Vite
  packages.
...
- Exact OpenBot-specific gap: a small main process, a typed preload API, role-aware onboarding,
  local service status/control adapters, Server connection management, and packaging gates that
...
- Files and upstream locations: this change records architecture and public upstream contracts only.
  The later implementation will use published Electron and Forge packages through their APIs.
