...
| 员工档案 | 七视图个人主页、可检查证据的 Owner 技能审核、带版本冲突检查和无内容审计的 Owner 管理分类记忆、安全模板导出、隔离导入、审核后生成新身份，以及实验性 DSSE 签名 | 记忆检索/保留和自主提案、系统钥匙串/KMS 与公开信任适配器、带完整 diff 审核的可执行 Agent Skills 包、选择性复制、注册表分发和所有权转移 |
