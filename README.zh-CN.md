...
OpenBot 当前仍是只有源代码的 pre-alpha 代码库。它已经跑通“本地频道 → 远程执行 Node →
结果回到频道”的受测试垂直切片，但尚无公开发布的桌面或移动客户端、可安装的工作主机发行包，
GitHub Packages 中也没有 OpenBot 产物。仓库唯一的 GitHub Release 是更早的纯源码
`v0.1.0-alpha.1` 基础预览版；它不含二进制资产，也不包含当前代码库。当前代码需要从源码运行。
下表刻意区分已落地代码与未来能力；托管 CI 兼容性不等于真实设备支持。
...
以下内容描述预期推出顺序，不代表当前已有可下载产品或支持承诺：

1. **Web 与 PWA 访问。** 桌面和移动浏览器继续共用一套响应式频道 Client，随后加入安装、通知、
   远程审批、恢复和独占接管。独立原生桌面或移动客户端尚不是已承诺里程碑；先交付可安装 PWA。
2. **原生工作主机。** 完成 Linux systemd、Windows Service 与 macOS launchd 生命周期，使用受
   操作系统保护的凭证存储，支持可恢复升级，并在宣称平台受支持前取得明确的真实设备证据。
3. **Agent/runtime 与 Provider 适配器。** 浏览器、桌面和隔离 coder 集成只能在 Server 授权的
   capability、策略、审批、审计和沙箱边界之后加入。当前有类型的 Cua、Lume 与 coder 包只是这些
   后续工作的扩展边界。
4. **公开分发。** 只有相关 `node-v*` 工作流从审核过的 tag 实际运行，且产物具备可复现性、来源
   证明、签名/公证、SBOM、安装、升级、回滚和恢复证据后，才发布可安装工作主机或 GitHub
   Packages。工作流定义在 CI 通过并不足够。
