...
describe(
  "Server-owned model configuration bootstrap",
  ...
  () => {
    it("retains one private key and decryptable opt-in configuration across restarts", async () => {
      ...
      expect(await readFile(join(directory, "settings.json"), "utf8")).not.toContain(
        "test-private",
      );
    ...
    it("does not replace a missing key beside retained ciphertext", async () => {
      ...
      await expect(stat(join(directory, "encryption.key"))).rejects.toMatchObject({
        code: "ENOENT",
      });
