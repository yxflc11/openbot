...
| Employee profile | Seven-view profile, Hermes-inspired dated evolution archive with filters and full evidence references, inspectable Owner skill review, Owner-managed typed memory with revision checks and content-free audit, safe template export, quarantined import, reviewed fresh-identity activation, and experimental DSSE signing | Memory retrieval/retention and autonomous proposals, native keyring/KMS and public trust adapters, executable Agent Skills bundles with full-diff review, selective cloning, registry distribution, and ownership transfer |
